...
def benchmark_to_disk(benchmark_results, test_type = "layer"):
    df = pd.DataFrame(benchmark_results)
    output_path = os.path.join("output", "3_nt_rvpnot_tlm","benchmark_sm1.csv")
    os.makedirs(os.path.join(*Path(output_path).parts)[:-1], exist_ok=True)
    df.to_csv(output_path, index=False)


def test_SHAiningTask():
    ...
    CONTESTANTS = ['sm1', 'ilp', 'inductive']#, 'sm2', 'imf', 'heuristics']
    ...
    SHAINING_PARAMS = {"feature_names": FEATURE_NAMES,
                       "feature_values": VALUE_LIST,
                       "contestants": CONTESTANTS,
                       "system_params": SYSTEM_PARAMS}
    results = SHAiningTask(params=SHAINING_PARAMS).shapley_results

    assert results is not None
    assert results.shape == (216, 5)
    assert set(results.columns.to_list()) == {'metric', 'algorithm', 'feature_value', 'shapley_value', 'feature_name'}
    assert results.isna().any().sum() == 0
    assert set(results['algorithm'].unique()) == set(CONTESTANTS)
    assert set(results['metric'].unique()) == {'fitness', 'pnsize', 'precision', 'benchtime', 'fscore', 'cfc', 'size', 'exectime'}
    assert set(results['feature_name'].unique()) == set(FEATURE_NAMES)
    assert set(results['feature_value'].unique()) == {'10', '9984', '119', '12', '990', '099'}
    #TODO: Run once with empty data and output dirs to update BENCHMARK_RESULTS_FOR_DISK and FEATURES_FOR_DISK
    """
    assert results.describe().round(2).to_dict() == {'shapley_value': {'count': 96.0, 'mean': 6.74,
                                                                       'std': 19.14, 'min': -34.85,
                                                                       '25%': 0.28, '50%': 2.08,
                                                                       '75%': 7.42, 'max': 98.32}}
    """
