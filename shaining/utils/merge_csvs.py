...
from shaining.utils.io_helpers import sort_files
...
def merge_csvs(input_dir, output_csv):
    ...
    filename_list = [filename for filename in filename_list if filename.startswith(name_prefix) and filename.endswith(".csv") and filename.startswith("benchmark_")]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Convert multiple CSVs in a directory to a CSV file.')
    parser.add_argument('csv_dir', type=str, help='The directory containing CSV files to merge')
