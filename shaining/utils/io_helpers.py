...
def delete_directory_contents(directory, exception_files = []):
    ...
    for filename in os.listdir(directory):
        ...
        if filename.split(".")[0] not in exception_files:  # Check if the file is not in the exception list
...
def delete_list_of_dirs(dir_list: list):
    for directory in dir_list:
        delete_directory_contents(directory)

def _results_already_exists(results_path, required_ELnames):
    if required_ELnames is None:
        raise ValueError("ERROR: required_ELnames cannot be None")
    if os.path.exists(results_path):
        df = pd.read_csv(results_path)
        df = df.dropna()
        if df.shape[0] > 0 and LOG in df.columns:
            if set(required_ELnames).issubset(set(df[LOG].to_list())):
                return True
            else:
                return False
    else:
